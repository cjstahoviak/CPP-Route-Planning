...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node)
{
    ...
    {
        if (n != current_node->parent && !n->visited)
        {
...
void RoutePlanner::DijkstraSearch()
{
    ...
    //std::priority_queue <RouteModel::Node*> pq;

    auto compare = [](RouteModel::Node * a, RouteModel::Node * b)
                {
                    return a->dijkstra_dist > b->dijkstra_dist;
                };
    std::priority_queue <RouteModel::Node *, std::vector<RouteModel::Node *>, decltype(compare)> pq(compare);
        ...
        for(RouteModel::Node *n : current_node->neighbors) {
            if(!n->dij_visited) {
                ...
                if(n->dijkstra_dist == -1 || n->dijkstra_dist > relative_dist) {
                    n->dijkstra_dist = current_node->dijkstra_dist + current_node->distance(*n);
                    n->parent = current_node;
                    pq.push(n);
                }
            }
